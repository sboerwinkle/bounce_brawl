...
void lvltutorial(){
	...
	tasktextadd(250, -170, "Hold 'Left Ctrl' to set your spawn\nHold 'L. Shift'+'L. Ctrl' to respawn");
	tasktextadd(-30, -90, "Welcome!\nUse WASD to move.\nI'd try 'A' first.\nYou'll get the hang of it.");
	tasktextadd(230, -90, "Try holding 'Left Shift'\nwith 'A' or 'D' to\nroll.");
	tasktextadd(500, -90, "Press 'Left Ctrl' to interact with\nobjects marked with squares.\nPress 'L. Shift' to fire this gun.");
...
void lvlbigplanet(){
	...
	register int i = 0;
	for(; i < players; i++){
		taskguycontroladd(-10-520*cos(i*rads), -10+520*sin(i*rads));
	}
